...
namespace EvolveGames
{
    public class ItemChange : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            // Ensure ItemIdInt is within the valid range for ItemLogos array
            if (ItemIdInt >= 0 && ItemIdInt < ItemLogos.Length)
            {
        ...
        private void Update()
        {
            ...
            {
                // Trigger swing animation for the current weapon
        ...
        IEnumerator ItemChangeObject()
        {
            ...
            {
                // Ensure i is within the valid range for the Items array
            ...
            // Ensure ItemIdInt is within the valid range for the Items array
        ...
        // Method to trigger the swing animation for the current weapon
        IEnumerator SwingWeapon()
        {
            // Check if the current weapon has an Animator
            if (Items[ItemIdInt].TryGetComponent(out Animator weaponAnimator))
            {
                // Trigger the swing animation
                weaponAnimator.SetTrigger("SwingTrigger");

                // Wait for the animation to complete
                yield return new WaitForSeconds(weaponAnimator.GetCurrentAnimatorClipInfo(0).Length);
            }
        }
    }
}
